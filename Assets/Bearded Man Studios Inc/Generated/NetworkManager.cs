...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] MatchManagerNetworkObject = null;
		public GameObject[] NetPlayerNetworkObject = null;
		public GameObject[] NetworkCameraNetworkObject = null;
		public GameObject[] Player_NetworkObject = null;
		public GameObject[] ProjectileNetworkObject = null;
		public GameObject[] ProjectilePoolNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is MatchManagerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (MatchManagerNetworkObject.Length > 0 && MatchManagerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(MatchManagerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<MatchManagerBehavior>();
						}
					}
			...
			else if (obj is NetPlayerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (NetPlayerNetworkObject.Length > 0 && NetPlayerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(NetPlayerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<NetPlayerBehavior>();
						}
					}
			...
			else if (obj is Player_NetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (Player_NetworkObject.Length > 0 && Player_NetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(Player_NetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<Player_Behavior>();
						}
					}
			...
			else if (obj is ProjectilePoolNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (ProjectilePoolNetworkObject.Length > 0 && ProjectilePoolNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(ProjectilePoolNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<ProjectilePoolBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateMatchManager instead, its shorter and easier to type out ;)")]
		public MatchManagerBehavior InstantiateMatchManagerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(MatchManagerNetworkObject[index]);
			var netBehavior = go.GetComponent<MatchManagerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<MatchManagerBehavior>().networkObject = (MatchManagerNetworkObject)obj;

			FinalizeInitialization(go, netBehavior, obj, position, rotation, sendTransform);
			
			return netBehavior;
		}
		[Obsolete("Use InstantiateNetPlayer instead, its shorter and easier to type out ;)")]
		public NetPlayerBehavior InstantiateNetPlayerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(NetPlayerNetworkObject[index]);
			var netBehavior = go.GetComponent<NetPlayerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<NetPlayerBehavior>().networkObject = (NetPlayerNetworkObject)obj;
		...
		[Obsolete("Use InstantiatePlayer_ instead, its shorter and easier to type out ;)")]
		public Player_Behavior InstantiatePlayer_NetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(Player_NetworkObject[index]);
			var netBehavior = go.GetComponent<Player_Behavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<Player_Behavior>().networkObject = (Player_NetworkObject)obj;
		...
		[Obsolete("Use InstantiateProjectilePool instead, its shorter and easier to type out ;)")]
		public ProjectilePoolBehavior InstantiateProjectilePoolNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(ProjectilePoolNetworkObject[index]);
			var netBehavior = go.GetComponent<ProjectilePoolBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<ProjectilePoolBehavior>().networkObject = (ProjectilePoolNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of MatchManager
		/// </summary>
		/// <returns>
		/// A local instance of MatchManagerBehavior
		/// </returns>
		/// <param name="index">The index of the MatchManager prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public MatchManagerBehavior InstantiateMatchManager(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(MatchManagerNetworkObject[index]);
			var netBehavior = go.GetComponent<MatchManagerBehavior>();
		...
		/// <summary>
		/// Instantiate an instance of NetPlayer
		/// </summary>
		/// <returns>
		/// A local instance of NetPlayerBehavior
		/// </returns>
		/// <param name="index">The index of the NetPlayer prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public NetPlayerBehavior InstantiateNetPlayer(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(NetPlayerNetworkObject[index]);
			var netBehavior = go.GetComponent<NetPlayerBehavior>();
		...
		/// <summary>
		/// Instantiate an instance of Player_
		/// </summary>
		/// <returns>
		/// A local instance of Player_Behavior
		/// </returns>
		/// <param name="index">The index of the Player_ prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public Player_Behavior InstantiatePlayer_(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(Player_NetworkObject[index]);
			var netBehavior = go.GetComponent<Player_Behavior>();
		...
		/// <summary>
		/// Instantiate an instance of ProjectilePool
		/// </summary>
		/// <returns>
		/// A local instance of ProjectilePoolBehavior
		/// </returns>
		/// <param name="index">The index of the ProjectilePool prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public ProjectilePoolBehavior InstantiateProjectilePool(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(ProjectilePoolNetworkObject[index]);
			var netBehavior = go.GetComponent<ProjectilePoolBehavior>();
